	...
	\label{section:cross-benchmarks}
		...
		\subsection{Back-end}
			...
			\begin{paragraph-here}% 9
				We can see in the results, especially looking at the geometric mean, that the performance of both Pyckets are quite similar, validating our hypothesis, that even though the evaluation of the program expansion is included in the computation (but not the measured time), the back-end performance remains mostly unaffected. In some of the benchmarks (such as `paraffins`, `earley`, `nboyer`, and `perm9`) we see a slow-down because EXPLANATION.
			\end{paragraph-here}
		...
		\subsection{Front-end}
			...
			\begin{paragraph-here}% 11
				Now we take a look at the run times for the expansion of the same benchmarks. For this, we used the same setup that's described in the \secref{benchmark-setup}. And turned the benchmarks into benchmarks of expansion.
			\end{paragraph-here}

			\begin{figure-here}
				FIGURE -- ack vs ack-expand side by side Racket code.
			\end{figure-here}

			\begin{paragraph-here}% 12
				\figref{ack-vs-ack-expand} shows the transformation. Each benchmark explicitly calls `expand` repeatedly and the time is measured in-system. Although these benchmarks might not be the ideal benchmarks for benchmarking program expansion performance per se, we still wanted to use the same suite just to give a consistent and complete analysis of front-end and back-end performance of the same system with least amount of variables in experiments.
			\end{paragraph-here}
		...
		\begin{paragraph-here}
			\figref{fig:pycket-npww-opww-relR} shows ....
