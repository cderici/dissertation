\chapter[\texorpdfstring{PERFORMANCE EVALUATION OF SELF-HOSTING ON META-TRACING}
	...
	\label{section:memory}
		...
		\begin{paragraph-here}
			Another major issue that we identified as one of the essential problems
			in self-hosting Racket on meta-traced CEK is the \gls{gc} pressure caused by
			the long chains of continuation allocations on the heap. As we
			described in \chapterRef{chapter:pycket}, given a Racket module, Pycket now
			first runs the expander to fully expand and load the language before expanding and running
			the module. Since the expansion of a given Racket module is included in
			Pycket's run-time now, the JIT suffers from numerous heap
			...
			pressure for the garbage collection, as the \gls{gc} is triggered by
			allocations. To understand the issue better, we start by briefly
			describing the \gls{gc} utilized by the RPython framework.
		\end{paragraph-here}

		\begin{paragraph-here}
			As described in \chapterRef{chapter:rpython}, the flow graphs and the C
			...
			program is linked with a \gls{gc} that is implemented within the framework
			...
			bolzPhDThesis, gc:16, LuaJITLanguageToolkit}
		\end{paragraph-here}
		...
		\begin{table}[!h]
			...
			Pycket with Self-hosting     & 500  &   546.4 & 148.0 & 890.6\\
			\bottomrule
			\end{tabular}
			\caption{Memory‑management footprint of self‑hosted Pycket vs original Pycket. Nursery size: 32M}
			\label{table:memory-footprints}
		\end{table}

		\begin{paragraph-here}
			\tableRef{table:memory-footprints} shows values All values are averages over a 1000 runs.
			Switching to a self-hosting build significantly impacts the memory-management behavior of Pycket, particularly affecting RPython's Minimark garbage collector. Compared to the non-self-hosted baseline (the original Pycket), the self-hosted version increases minor garbage collections dramatically from 15 to 500. Although each minor collection in self-hosting Pycket copies fewer bytes (546 KiB vs. 1367 KiB), the cumulative effect is substantial, causing an approximately seven-fold increase in old-generation growth (148.0 MiB vs. 19.2 MiB). Consequently, GC wall-time escalates significantly, from 0.04 seconds in the original Pycket to 0.89 seconds in the self-hosting Pycket, confirming that the large, pointer-rich Racket structures introduced by self-hosting significantly challenge Minimark's design assumptions.
		\end{paragraph-here}

		\inputFigure{problem}{nursery-size-vs-gc-time-figure}

		\begin{paragraph-here}
			Figure~\ref{figure:nursery-size-vs-gc-time} illustrates the direct impact of nursery size on the garbage collection overhead introduced by self-hosting. Initially, small nursery sizes (2--8\,MiB) trigger frequent minor collections, accumulating significant GC overhead (1.7--1.0\,s). Increasing the nursery size to match or slightly exceed the CPU cache size (32--64\,MiB) effectively reduces GC overhead to approximately 0.5\,s, providing a clear performance sweet-spot. Beyond this range, further increases in nursery size yield diminishing returns and can eventually lead to unrealistic memory usage, as evidenced by the sharp reduction in GC time at impractically large sizes ($\ge$ 2\,GiB). These results reinforce our earlier observation that self-hosting generates numerous long-lived, pointer-rich Racket structures, challenging Minimark's generational assumptions, and demonstrate that tuning nursery size to hardware characteristics can mitigate—but not fully eliminate—the associated memory management overhead.
		\end{paragraph-here}
