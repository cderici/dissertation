\chapter{Performance Impact of Self-Hosting on a Meta-Tracing JIT}
	...
	\label{section:nature-of-the-beast}
		...
		\subsection{Performance of Program Expansion}
			...
			\paragraph{}%
				Recall that PyPy addresses tracing challenges by identifying and tracing loops directly in the user program rather than at the interpreter level. Essentially, PyPy elevates the abstraction level at which the essential computational patterns are captured in machine code~\cite{pypy-main}.

			\begin{figure}[h] % let LaTeX decide where it fits best
				\centering
				\includegraphics[width=0.7\linewidth]{\inputFigPath{problem}{theres-no-loop.png}}
				\caption{We put a branchy expander in front of the interpreter.[PLACEHOLDER IMAGE]}
				\label{fig:theres-no-loop}
			\end{figure}

			\paragraph{}%
				\figref{fig:theres-no-loop} shows the architectural impact of integrating Racket's expander directly in front of Pycket's interpreter. With this design, the interpreter evaluates an expansion program rather than a conventional user-level program. Thus, when tracing the expander stage, the meta-tracer seeks hot loops within a higher abstraction layer—where loops are fundamentally sparse or overly complicated, and typical programs become mere streams of heterogeneous inputs, severely limiting opportunities for beneficial loop specialization.

			\paragraph{}%
				Transforming or expanding a program shares fundamental characteristics with regular expression matching or FASL parsing—each being equally data-dependent. The essence of the computation lies in the ordered manipulation and parsing of symbolic structures rather than semantics within the input. [We need something here that explains expanding/transforming a user program that contains a loop is an entirely different computation to a tracing JIT than interpreting it]  The loops within a fully expanded user program remain efficiently capturable by Pycket's meta-traced CEK machine core. We will discuss the implications of this multi-level tracing scenario and its resulting performance characteristics further in \sectionRef{section:cross-benchmarks}.
			...
			\paragraph{}%
				Clearly, addressing this challenge requires extensive research and careful consideration. Whereas PyPy strategically relocates the computational tracing to a higher abstraction, effectively shifting the complexity upward, the self-hosting Pycket encounters these tracing difficulties directly within the interpreter expansion layer—precisely where the core issue inherently arises. At the heart of this challenge is the fundamental violation of the second assumption underlying tracing JITs, as previously discussed in \chapterRef{chapter:rpython}. The second assumption—that multiple iterations of the same loop are likely to traverse similar code paths—is directly contradicted by the highly data-dependent, branch-heavy nature of interpreter-style computations such as those found in the expander. This inherent violation explains why such computations, although critical for self-hosting, remain particularly ill-suited to meta-tracing and tracing JITs in general.
