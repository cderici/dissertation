\chapter{Performance Impact of Self-Hosting on a Meta-Tracing JIT}
	...
	\label{section:module-and-language-loading}
		...
		\paragraph{}%
			Regardless of the source of each module, every macro expansion of user code ultimately depends on the performance of the underlying expander. Pycket's ability to inline into, and sometimes across, expander helpers opens new optimization opportunities, but it also means that deficiencies in the expander's own implementation now directly affect application start-up. The next sections discuss those interactions in detail.
	...
	\label{section:nature-of-the-beast}
		...
		\begin{paragraph-here}
			...
			[This is basically a connector paragraph that takes the reader's hand from the previous section and pulls him into this one.]
		\end{paragraph-here}
		...
		\centering
		\includegraphics[width=\linewidth]{\inputFigPath{problem}{branchy-introduce-code.png}}
		...
		\footnote{Full code of branchy is available at \url{https://github.com/cderici/pycket-performance/blob/master/branchy/private/branchy.rkt}.}
		...
		\begin{show-experiment}
			Now run branchy with an input that repeatedly takes a straight path.

			Show the path it takes in the source code. And show the trace it generates.
		\end{show-experiment}

		\begin{paragraph-here}
			See how fast it goes. It's still unique to this input, therefore not reusable.
		...
		\begin{paragraph-here}
			So what do we want to see in a trace for computations on branchy? Discuss branching in the tracing JITs, with guards and bridges. Do we want a trace per branch? Do we want a one giant trace that has all the branches in it?
		\end{paragraph-here}
