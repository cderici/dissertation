...
\end{lstlisting}
    ...
    % sub‑caption A
    \node (captionA) at ([yshift=-1cm]A.south) {(A) Back-end benchmark};

    % right listing (B)
    \node[anchor=north west] (B) at ($(A.north east)+(1.0cm,0)$) {%
      \begin{minipage}{0.4\linewidth}
\begin{lstlisting}[language=racket,
                   basicstyle=\ttfamily\footnotesize,
                   numbers=none,
...
\end{lstlisting}
    ...
    \node (captionB) at ([yshift=-1cm]B.south) {(B) Front-end benchmark};
    
    % centre divider
    \draw[very thick,gray!50]
          ($(A.north east)+(0.2cm,0.2cm)$) -- ++(0,-9cm);
