...
\label{chapter:linklets}
	...
	\section[\texorpdfstring{Linklets as Units of Compilation}{Semantics for Linklets}]{Linklets as Units of Compilation}
		...
		\begin{paragraph-here}
			Regular instantiation is where a linklet is given some linklet instances for its imported variables and evaluates the expressions within its body. As illustrated in \figref{fig:basic-linklet-instantiation}, the result of a regular instantiation is a linklet instance, which is container for a mapping of the exported variables to values, along with some other data (e.g. namespace).
		\end{paragraph-here}

		\begin{paragraph-here}
			A targeted instantiation on the other hand is where a linklet is given a \emph{target} instance along with some other linklet instances for its imported variables. This kind of instantiation is often used for side-effects, and the result is the value of the last expression in the linklet (instead of a fresh linklet instance).
		\end{paragraph-here}
