...
\label{chapter:linklets}
	...
	\begin{paragraph-here}
		The general anatomy of a practical, real programming language, has:
			...
			- core langauge semantics: what you mean by what you express -- reduction semantcs -- rules that takes you from an expression to a value

			- runtime: the machinery that allows you to realize the reduction semantics on a computing environment. You need to be able to read and expand expressions (allow users to make expressions in the langauge), print values (allow users to evaluate expressions and receive values)..
	\end{paragraph-here}
...
% This is not only about Racket, the CPython is written in C. Python developers don't want to write C, they
	...
	\section[\texorpdfstring{Enriching Compiler \& Runtime Communication}{Motivation for Linklets}]{Enriching Compiler \& Runtime Communication}
		...
		\begin{paragraph-here}
			There's a tight coupling between the runtime and the reduction semantics in a programming language.
				...
				- You may have a certain way to serialize values/objects of your language into the file system to unmarshall it later back into the memory. Runtime needs to know how.
		...
		\begin{figure-here}
			SHOW THE FIGURE with 3 stacks

			\begin{todo}[TECH TODO]
				latexify that figure
			\end{todo}
		\end{figure-here}

		\begin{paragraph-here}
			Racket turned everything on the stack from C to Racket, notably the expander, which implements macro system, module system, read, expand, basically most of the high-level stuff you need for reading and expanding Racket code in any language that Racket macro expander can elaborate.
		\end{paragraph-here}

		\begin{figure-here}
			SHOW THE LINKET EXPANSION FIGURE

			\begin{todo}[TECH TODO]
				grab and prep the figure
			\end{todo}

			Don't forget to cite in caption.
		\end{figure-here}
	...
	\section[\texorpdfstring{Linklets as Units of Compilation}{Semantics for Linklets}]{Linklets as Units of Compilation}
		...
		\begin{paragraph-here}
			In this section, we introduce linklets as a language form, the minimal API for a VM to implement to support linklets, their operational semantics, along with a practical example of how they implement interesting functionality.
		\end{paragraph-here}

		\begin{figure-here}
			Linklet grammar (quals doc Fig.1)

			Maybe side by side with an example linklet (see quals Fig.2)

			\begin{todo}[TECH TODO]
				grab and prep the figure, possibly make a BNF with latex instead of an actual picture.
			\end{todo}
		\end{figure-here}

		\begin{paragraph-here}
			The form....A linklet consists of a set of variable definitions and expressions ....... 2nd paragraph in 3.2.1 proposal

			refer to the figure
		\end{paragraph-here}

		\begin{figure-here}
			Basic instantiation of a linklet (quals Fig 3)

			\begin{todo}[TECH TODO]
				grab and prep the Fig 3 from quals, possibly make a latex tikz figure for it
			\end{todo}
		\end{figure-here}
