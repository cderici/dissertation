...
\label{chapter:linklets}
	...
	\section[\texorpdfstring{Formal Operational Semantics of Linklets}{Formal Semantics for Linklets}]{Formal Operational Semantics of Linklets}
		...
		\begin{paragraph-here}% 4
			...
			in the run-time. \figref{fig:linklet-runtime-language} illustrates the runtime representations of compiled linklets and linklet instances. For each imported and exported identifier (also keeping
		...
		\begin{paragraph-here}% 5 - ONLY PHRASING
			...
			the linklet body one by one. We're only interested in compiling forms related to linklets, so we omit, for the sake of clarity, any compilation details of turning s-expressions of basic forms (e.g. \racketcode{begin}, \racketcode{set!}) into runtime \glspl{ast}.
		\end{paragraph-here}

		\inputFigure{linklets}{compile-linklet-transformation-rules-table}

		\begin{paragraph-here}
			\tableRef{table:linklet-compilation-rules} lists the transformation rules for the linklet variables used in \racketcode{compile-linklet} semantics. Any exported and top-level defined identifier is related to a linklet variable that'll get created at the instantiation. An exported identifier that's a target of a \racketcode{set!} is turned to be mutated by a \racketcode{var-set/check-undef!} operation. Imported
		\end{paragraph-here}
