  ...
  \fbox{%
    \begin{minipage}{0.93\textwidth}
      ...
      \begin{align*}
        ...
        \;          &\;\; \hspace{1.2cm} \where x \in \rho, \rho[x] \in \sigma, v = \sigma[\rho[x]],\\
        \;          &\;\; \hspace{2.3cm} v \neq uninit\\
        \rcinput{(\textbf{var\dash ref/no\dash check}\; x))} \rcrel \rcoutput{v}{\rho}{\sigma}\; \where v = \sigma[\rho[x]] \\
        \rcinput{(\textbf{var\dash set!}\; x\; v)} \rcrel \rcoutput{(void)}{\rho}{\sigma_1} \\
        \;          &\;\; \hspace{1.2cm} \where \sigma_1 = \sigma[\rho[x]\rightarrow v]\\
        \rcinput{(\textbf{var\dash set/check\dash undef!}\; x\; v)} \rcrel \rcoutput{(void)}{\rho}{\sigma_1} \\ 
        \;          &\;\; \hspace{1.2cm} \where \sigma_1 = \sigma[\rho[x]\rightarrow v], x \in \rho\\
        \rcinput{(\textbf{set!}\; x\; v)} \rcrel \rcoutput{(void)}{\rho}{\sigma_1} \\ 
        \;          &\;\; \hspace{1.2cm} \where \sigma_1 = \sigma[\rho[x]\rightarrow v]\\
      \end{align*}
    \end{minipage}%
  }
  \caption{Standard Reduction Relation $\longrightarrow_{\beta_l}$ for \gls{lkl} (Redacted for clarity).}
  \label{fig:linklets-reduction-relation-beta-l}
\end{figure}
