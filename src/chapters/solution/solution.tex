\chapter[\texorpdfstring{APPROACHES TO IMPROVE SELF-HOSTING PERFORMANCE}
  ...
  \paragraph{}%
    ...
    \begin{paragraph-here}% 12
      \figref{fig:approaches-experiment} shows the overall run times for running Branchy with all 0 input (takes the deep branch path), all 20 (takes the shallow deep branch), and randomized input, with meta-hints and extended green variables, along with bootstrapped confidence intervals at the 95\% confidence level~\cite{davisonBootstrapMethods2013}. The input sizes are 2000 numbers each. All run times are averages over 1000 runs with the same setup explained in \sectionRef{benchmarkSetups}. The random inputs are generated freshly at each iteration, and kept the same across the interpreters at each run.
    \end{paragraph-here}
