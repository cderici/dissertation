\chapter[\texorpdfstring{APPROACHES TO IMPROVE SELF-HOSTING PERFORMANCE}
  ...
  \paragraph{}% 7
    ...
    \begin{paragraph-here}% 22.1
      Although these methods proposed here are very rudimentary approaches that target specific scenarios, it is logical to at least try to apply these to the general self-hosting scenario where we run Racket's expander on real programs to see if the approaches help make Pycket go faster than vanilla Pycket with no optimizations. Recall that one of the downsides of the meta-hints approach is that the input program needs to be manually annotated. Since the Racket expander is quite large and different ways of annotating the program directly effects the strategy and therefore the performance, it is difficult to extract useful insights on the effectiveness of the approach for the general self-hosting scenario from a single experiment with only one way of annotating without performing a thorough analysis of the effects of different ways of annotating the user code. Therefore, here we only experiment with the extended green variables approach. Recall that in \secref{section:cross-benchmarks} we measured the performance of expansion hosted on the new Pycket against Racket over a benchmark suite designed to measure expansion times. Using the same experiment setup we described in \secref{benchmark-setup}, here we compare performances over the same benchmark suite on Pycket with extended green variables optimization vs vanilla Pycket with self-hosting, also keeping Racket results as well. Note that these benchmarks measure the expansion times for each named benchmark, therefore, the performance has nothing to do with the actual program the benchmark is designed for. For instance, the actual runtime for the \emph{fft} benchmark in the back-end shown in \figref{fig:pycket-npww-opww-relR} is close to ten times better than Racket. However, the expansion time is about two orders of magnitude slower. For the expansion performance, the benchmarks that make the expander take shallow paths more than the deeper paths repeatedly will perform better.
    \end{paragraph-here}

    \inputFigure{solution}{approaches-green-vars-expansion}

    \begin{paragraph-here}% 22.2
      \figref{fig:crossbenchmark-expansion-times-vs-R-green-vars} shows the results. We observe across the board the Pycket with the optimization is generally slower, like less than half an order of magnitude slower than the vanilla Pycket, because the approach is designed to target a specific scenario, and needs further development and fine-tuning for the general case. However, it does seem to perform slightly better in several cases where we have more complex program structure with convoluted code and syntactic indirections. These cases--including \emph{gcold}, \emph{earley}, and \emph{sboyer}--make the expander handle more cases and therefore makes the computation more branch-heavy and puts pressure on the tracer. We observe in the expansion performance results for these cases that the extended green variables approach successfully takes some of that pressure away. As a part of improvement for this approach, further analysis is needed to determine what exactly is going on within the expander itself during the expansion of each of these programs to fully understand the effects of using the depth information as a heuristic for attempting to steer the tracer away from more branch-heavy computations.
    \end{paragraph-here}
