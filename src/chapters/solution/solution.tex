\chapter[\texorpdfstring{APPROACHES TO IMPROVE SELF-HOSTING PERFORMANCE}
  ...
  \paragraph{}% 7
    ...
    \begin{itemize}
      ...
      \item[B.] A regexp match scenario designed specifically to create a deeper, heavily branched loop. The matcher attempts to match the pattern $\mathtt{\#}$\racketcode{rx"abcdefghijklmnopqrstuvwxyz"} against the input string \racketcode{"abcdefghijklmonpqrstuvwxy(*1000)...abcdefghijklmnopqrstuvwxyz"}. Notice that the prefix repeatedly excludes the final character "z", forcing deep branching during matching until the pattern is finally matched at the very end.
