  ...
  \paragraph{}%
    ...
    \subsection{CEK \+ Stackful Prototype on Pycket}
      ...
      \begin{paragraph-here}% 7
        The central investigative goals for this prototype revolve around confirming reduced memory overhead, minimal interpreter-switching overhead, and bounded copying of activation records from the stack to the heap. To assess these goals, we conducted experiments using the \texttt{Branchy} program from \chapterRef{chapter:problem} (see \figref{fig:branchy-introduce-code}). We executed \texttt{Branchy} on Pycket under two configurations: the hybrid Stackful+CEK interpreter and the CEK-only vanilla interpreter. We used the same inputs that we used for the experiments in \secref{section:hot-branches}; one that exclusively takes the shallow Exit A, one that takes the deeper Exit B, and input with randomized sequences, with all inputs having 2000 numbers. Similar to the earlier experiments, they're all ran with the setup detailed in \secref{benchmark-setup} and all results are averages over 1000 runs with bootstrapped confidence intervals at the 95\% confidence level~\cite{davisonBootstrapMethods2013}. Note that \texttt{Branchy} program-intentionally-doesn't involve any continuation primitives, to isolate the measurement to only the run-time allocated continuations. The hybrid model starts the computation at the CEK interpreter, and switches to the stackful when it encounters the first \racketcode{let} form, and continues from there.
      \end{paragraph-here}

      \inputFigure{solution}{cek-stackful-overall-runtime}

      \begin{paragraph-here}% 8
        \figref{fig:hybrid-model-overall-good} presents median runtime results. One slight overhead that we observe in all experiments is higher warmup times for the hybrid model. This is expected, as RPython's meta-tracing \gls{jit} is not designed for recursive interpreter structures, and that needs to be optimized in isolation. The CEK in Pycket's core has been under improvement for years, while the stackful interpreter is only a primitive prototype. Nevertheless, we see comparable performance when combined with the CEK. For the experiment that takes the shallow branch path, we see a slight slowdown due to warmup and the overhead of switching, but still comparable. We observe that that difference closes when we run on deeper branching computations. Notably, we observe a consistently improved run time performance in the hybrid configuration during experiment running randomized inputs. This is due to the reduced memory footprint as we further demonstrate below with memory analyses. Evidently, this suggests that this is indeed a viable approach that warrants further research.
      \end{paragraph-here}
      ...
      \begin{paragraph-here}% 9
        \tableRef{table:memory-footprints-cek-vs-hybrid} presents the memory footprint of CEK-only Pycket vs the hybrid Pycket with CEK \& Stackful, each running Branchy with randomized input. All values presented in the table are averages computed over 1000 runs during the benchmarks presented in \figref{fig:hybrid-model-overall-good}. We observe that using a stackful interpreter along with the CEK model dramatically decreases the frequency of minor garbage collections-from an average of 32 collections in the original CEK-only Pycket to 10 collections in the hybrid model. The amount of bytes that are copied is larger in the hybrid model, which is expected, since switching to the CEK requires re-building the whole continuation in the heap. And the old-generation heap growth is dramatically reduced, because i) the CEK doesn't run enough by itself to retain objects in the heap, it switches to the stackful, ii) in the stackful we don't need to keep large continuation objects. This explains the run time results we see in \figref{fig:hybrid-model-overall-good}.
      \end{paragraph-here}

      \begin{paragraph-here}% 10
        Thus, the preliminary evidence shows that this approach is a potential solution for mitigating memory pressure that's incurred by self-hosting. While this is a demonstration on a small prototype, this can be developed into a full hybrid model to further analyze the performance benefits in larger programs. This would also reveal interesting runtime behaviors for programs involving continuations, since for those the stackful interpreter will be forced to switch to CEK. This requires significant engineering effort, however, since each form needs to be handled individually, and proper error handling, retaining proper tail calls, while preserving the switch overhead bounded at all times, all present considerable challenges.
      \end{paragraph-here}
    ...
    \subsection{Formalism for CEK \+ Stackful}
      ...
      \begin{paragraph-here}% 11
        To facilitate deeper understanding and enable further research in this technique, we developed a formal model integrating both the CEK and stackful interpreters using PLT Redex. The basic interaction model depicted in \figref{fig:cek-convert-stack}. This formalism evaluates a simplified subset of Racket, closely resembling the Linklet Kernel language described previously in \secref{section:linklet-semantics}, but excluding the linklet variable forms. Additionally, this model introduces specialized \racketcode{convert} forms explicitly designed for managing interpreter transitions directly from the source code. \figref{fig:st-cek-grammar} illustrates this simplified language that allows for deliberate interpreter switching through these special forms.
      \end{paragraph-here}
